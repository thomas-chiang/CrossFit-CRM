...
async function _createFakeUser(conn) {
    ...
    await conn.query('INSERT INTO users (role, email, password, name, gender, valid) VALUES ?', [encryped_users.map(x => Object.values(x))])
}
...
async function _createFakeCourseUser(conn) {
    ...
    await conn.query('INSERT INTO course_user (course_id, user_id, is_coach) VALUES (?, ?, ?)', [first_course_id, first_member_id, 1])

    await conn.query('INSERT INTO course_user (course_id, user_id, enrollment) VALUES (?, ?, ?)', [first_course_id, first_coach_id, 1])
}
...
async function _createFakeWorkout(conn) {
    ...
    await conn.query('INSERT INTO workouts (name, round, extra_count, minute, extra_sec, note, creator_id) VALUES ? ', [workouts.map(x => Object.values(x))])
}
...
async function _createFakeWorkoutMovement(conn) {
    ...
    await conn.query('INSERT INTO workout_movement (movement_id, workout_id, kg, rep, meter, cal) VALUES ? ', [movements.map(x => Object.values(x))])
}
...
async function _createFakePerformance(conn) {
    ...
    await conn.query('INSERT INTO performances (workout_movement_id, workout_id, movement_id, kg, rep, meter, cal, course_id, user_id, round, extra_count, minute, extra_sec) VALUES ? ', [workoutMovements.map(x => Object.values(x))])
}
...
async function truncateFakeData() {
    ...
    const tables = ['users', 'courses', 'course_user', 'course_workout', 'movements', 'performances','points', 'semaphore', 'workout_movement', 'workouts'];
